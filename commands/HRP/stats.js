...
module.exports.run = (bot, message, args) => {
  ...
  const online = guild.members.cache.filter(member => member.user.presence.status === 'online').size;
  const idle = guild.members.cache.filter(member => member.user.presence.status === 'idle').size;
  const offline = guild.members.cache.filter(member => member.user.presence.status === 'offline').size;
  const dnd = guild.members.cache.filter(member => member.user.presence.status === 'dnd').size;
  const Robot = guild.members.cache.filter(member => member.user.bot).size;
  const AllMember=online+idle+offline+dnd+Robot;
  ...
  const servembeb = new MessageEmbed()
    ...
    .addField(`Nombre totale de membre du serveur : ***${AllMember}***`, `*soit ${Robot} bot\n${online - Robot} en ligne\n${idle} AFK\n${dnd} en ne pas déranger\net ${offline} hors-ligne*`)
