...
module.exports.run = async (bot, message, args, data, userInfo) => {
  ...
  async function LootMob(MOB) {
      ...
      const Nombre = [
        [nombre=Math.floor(Math.random() * 3) + 1,"Griffe",Griffe(nombre),Griffe(0)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Peau",Peau(nombre),Peau(0)],
        [nombre=1,"Viande",Viande(nombre),Viande(0)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Bois",Bois(nombre),Bois(0)],
        [nombre=Math.floor(Math.random() * 4) + 1,"Sabot",Sabot(nombre),Sabot(0)],
        [nombre=Math.floor(Math.random() * 3) + 1,"Pollen",Pollen(nombre),Pollen(0)],
        [nombre=Math.floor(Math.random() * 3) + 1,"Azurite",Azurite(nombre),Azurite(0)],
        [nombre=Math.floor(Math.random() * 4) + 1,"HerbeMedic",HerbeMedic(nombre),HerbeMedic(0)],
        [nombre=Math.floor(Math.random() * 5) + 1,"Dent",Dent(nombre),Dent(0)],
        [nombre=Math.floor(Math.random() * 5) + 1,"Ecaille",Ecaille(nombre),Ecaille(0)],
        [nombre=Math.floor(Math.random() * 6) + 3,"Azurite",Azurite(nombre),Azurite(0)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Diamant",Diamant(nombre),Diamant(0)],
        [nombre=Math.floor(Math.random() * 4) + 1,"Carapace",Carapace(nombre),Carapace(0)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Mandibule",Mandibule(nombre),Mandibule(0)],
        [nombre=1,"Crâne",Crâne(nombre),Crâne(0)],
        [nombre=Math.floor(Math.random() * 4) + 1,"Sang",Sang(nombre),Sang(0)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Aile",Aile(nombre),Aile(0)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Oreille",Oreille(nombre),Oreille(0)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Corne",Corne(nombre),Corne(0)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Oeil",Oeil(nombre),Oeil(0)]
      ];
