...
module.exports.run = async (bot, message, args, data, userInfo) => {
  if (userInfo.MOB.mob === "Loup" ||
    ...
    // LOUP
    if (userInfo.MOB.mob === "Loup") {
      const Channel = "568923274658971668";
      if (message.channel.id !== Channel) return message.channel
    ...
    if (userInfo.MOB.mob === "Pripom") {
      const Channel = "568937782630678541";
      if (message.channel.id !== Channel) return message.channel
    ...
    if (userInfo.MOB.mob === "Croco") {
      const Channel = "569505805561364511";
      if (message.channel.id !== Channel) return message.channel
    ...
    if (userInfo.MOB.mob === "Golem") {
      const Channel = "569207554404515841";
      if (message.channel.id !== Channel) return message.channel
    ...
    if (userInfo.MOB.mob === "Gimille") {
      const Channel = "569209205207072773";
      if (message.channel.id !== Channel) return message.channel
    ...
    if (userInfo.MOB.mob === "Varghulf") {
      const Channel = "569209235166986249";
      if (message.channel.id !== Channel) return message.channel
    ...
    if (Attaque >= 2 && Attaque <= 5) {
      ...
      const PvMob = userInfo.MOB.Pv;
      bot.updateUserInfo(message.member, {
        "users.$.MOB.Pv": PvMob - ForceUser
      });
      if (userInfo.MOB.Pv - ForceUser <= 0) {
    ...
    // Stats mob
    const userInfo2 = bot.getUser(message.member);
    if (userInfo2.MOB.Pv > 0) {
      const StatsEmbeb = new MessageEmbed()
        .setColor("#27d200")
      ...
      if (AttaqueMob >= 3 && AttaqueMob <= 5) {
        ...
        const PvUser = userInfo2.EXP.PvActu;
        bot.updateUserInfo(message.member, {
          "users.$.EXP.PvActu": PvUser - ForceMob
        });
    ...
    // Stats joueur
    const userInfo3 = bot.getUser(message.member);
    if (userInfo3.EXP.PvActu > 0) {
      const StatsJoueurEmbeb = new MessageEmbed()
        .setColor("#27d200")
        .setDescription(`Stats de ${message.member.displayName}`)
        .setThumbnail(message.author.avatarURL())
        .addField("Point de vie :", userInfo3.EXP.PvActu, true)
