...
module.exports.run = async (bot, message, args, data, userInfo) => {
  ...
  async function LootMob(MOB) {
      ...
      const Nombre = [
        [nombre=Math.floor(Math.random() * 3) + 1,"Griffe",Griffe(nombre),Griffe(-nombre)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Peau",Peau(nombre),Peau(-nombre)],
        [nombre=1,"Viande",Viande(nombre),Viande(-nombre)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Bois",Bois(nombre),Bois(-nombre)],
        [nombre=Math.floor(Math.random() * 4) + 1,"Sabot",Sabot(nombre),Sabot(-nombre)],
        [nombre=Math.floor(Math.random() * 3) + 1,"Pollen",Pollen(nombre),Pollen(-nombre)],
        [nombre=Math.floor(Math.random() * 3) + 1,"Azurite",Azurite(nombre),Azurite(-nombre)],
        [nombre=Math.floor(Math.random() * 4) + 1,"HerbeMedic",HerbeMedic(nombre),HerbeMedic(-nombre)],
        [nombre=Math.floor(Math.random() * 5) + 1,"Dent",Dent(nombre),Dent(-nombre)],
        [nombre=Math.floor(Math.random() * 5) + 1,"Ecaille",Ecaille(nombre),Ecaille(-nombre)],
        [nombre=Math.floor(Math.random() * 6) + 3,"Azurite",Azurite(nombre),Azurite(-nombre)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Diamant",Diamant(nombre),Diamant(-nombre)],
        [nombre=Math.floor(Math.random() * 4) + 1,"Carapace",Carapace(nombre),Carapace(-nombre)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Mandibule",Mandibule(nombre),Mandibule(-nombre)],
        [nombre=1,"Crâne",Crâne(nombre),Crâne(-nombre)],
        [nombre=Math.floor(Math.random() * 4) + 1,"Sang",Sang(nombre),Sang(-nombre)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Aile",Aile(nombre),Aile(-nombre)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Oreille",Oreille(nombre),Oreille(-nombre)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Corne",Corne(nombre),Corne(-nombre)],
        [nombre=Math.floor(Math.random() * 2) + 1,"Oeil",Oeil(nombre),Oeil(-nombre)]
      ];
        ...
        for (let k=0 ; k<Nombre.length ; k=k+1) {
          if (MOB.loot2[1] === Nombre[k][1]) {
            ...
            const LootEmbed = new MessageEmbed()
              .setColor("#27d200")
              .setDescription("Loots ramassez !")
              .addField(MOB.loot2[0], Nombre[k][0])
              .addField("Point de Maîtrisse gagner : ", ExpGain)
              .setTimestamp();
            message.channel.send(LootEmbed)
              .then(msg => msg.delete({ timeout: 180000 }))
