...
module.exports.run = async (bot, message, args, data, userInfo) => {
  ...
  async function LootMob(MOB) {
      ...
      const Nombre = [
        [nombre=0,"Griffe",Griffe(nombre),nombre=Math.floor(Math.random() * 3) + 1],
        [nombre=0,"Peau",Peau(nombre),nombre=Math.floor(Math.random() * 2) + 1],
        [nombre=0,"Viande",Viande(nombre),nombre=1],
        [nombre=0,"Bois",Bois(nombre),nombre=Math.floor(Math.random() * 2) + 1],
        [nombre=0,"Sabot",Sabot(nombre),nombre=Math.floor(Math.random() * 4) + 1],
        [nombre=0,"Pollen",Pollen(nombre),nombre=Math.floor(Math.random() * 3) + 1],
        [nombre=0,"Azurite",Azurite(nombre),nombre=Math.floor(Math.random() * 3) + 1],
        [nombre=0,"HerbeMedic",HerbeMedic(nombre),nombre=Math.floor(Math.random() * 4) + 1],
        [nombre=0,"Dent",Dent(nombre),nombre=Math.floor(Math.random() * 5) + 1],
        [nombre=0,"Ecaille",Ecaille(nombre),nombre=Math.floor(Math.random() * 5) + 1],
        [nombre=0,"Azurite",Azurite(nombre),nombre=Math.floor(Math.random() * 6) + 3],
        [nombre=0,"Diamant",Diamant(nombre),nombre=Math.floor(Math.random() * 2) + 1],
        [nombre=0,"Carapace",Carapace(nombre),nombre=Math.floor(Math.random() * 4) + 1],
        [nombre=0,"Mandibule",Mandibule(nombre),nombre=Math.floor(Math.random() * 2) + 1],
        [nombre=0,"Crâne",Crâne(nombre),nombre=1],
        [nombre=0,"Sang",Sang(nombre),nombre=Math.floor(Math.random() * 4) + 1],
        [nombre=0,"Aile",Aile(nombre),nombre=Math.floor(Math.random() * 2) + 1],
        [nombre=0,"Oreille",Oreille(nombre),,nombre=Math.floor(Math.random() * 2) + 1],
        [nombre=0,"Corne",Corne(nombre),nombre=Math.floor(Math.random() * 2) + 1],
        [nombre=0,"Oeil",Oeil(nombre),nombre=Math.floor(Math.random() * 2) + 1]
      ];
        ...
        for (let j=0 ; j<Nombre.length ; j=j+1) {
          if (MOB.loot1[1] === Nombre[j][1]) {
            ...
            const LootEmbed = new MessageEmbed()
              ...
              .addField(MOB.loot1[0], Nombre[j][3])
        ...
        for (let k=0 ; k<Nombre.length ; k=k+1) {
          if (MOB.loot2[1] === Nombre[k][1]) {
            ...
            const LootEmbed = new MessageEmbed()
              .setColor("#27d200")
              .setDescription("Loots ramassez !")
              .addField(MOB.loot2[0], Nombre[k][3])
              .addField("Point de Maîtrisse gagner : ", ExpGain)
              .setTimestamp();
            message.channel.send(LootEmbed)
              .then(msg => msg.delete({ timeout: 180000 }))
            ...
            for (let j=0 ; j<Nombre.length ; j=j+1) {
              if (MOB.loot2[1] === Nombre[j][1]) {
                ...
                const LootEmbed = new MessageEmbed()
                  .setColor("#27d200")
                  .setDescription("Loots ramassez !")
                  .addField(MOB.loot1[0], Nombre[k][3])
                  .addField(MOB.loot2[0], Nombre[j][3])
                  .addField("Point de Maîtrisse gagner : ", ExpGain)
                  .setTimestamp();
                message.channel.send(LootEmbed)
                  .then(msg => msg.delete({ timeout: 180000 }))
